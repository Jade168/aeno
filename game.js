// AENO V3.7 - 最終修復版：與index.html完全匹配，強制密碼登入，手機觸摸全兼容
const AENO_VERSION = "V3.7-FINAL";
...
function logSys(msg) {
    ...
    ui.sysLog.style.display = "block";
    setTimeout(() => {
        ui.sysLog.style.display = "none";
    }, 5000);
}
...
function rebindUIEvents() {
    ...
    if (ui.btnBuildMode) ui.btnBuildMode.onclick = () => {
        mode = "build";
        ui.btnBuildMode.classList.add("active");
        ui.btnUpgradeMode.classList.remove("active");
        logSys("🏗️ 切換到建造模式");
    };
    if (ui.btnUpgradeMode) ui.btnUpgradeMode.onclick = () => {
        mode = "upgrade";
        ui.btnUpgradeMode.classList.add("active");
        ui.btnBuildMode.classList.remove("active");
        logSys("⬆️ 切換到升級模式");
    };
    // 自動建造開關
    if (ui.btnAuto) ui.btnAuto.onclick = () => {
        globalSave.autoBuild = !globalSave.autoBuild;
        ui.autoState.textContent = globalSave.autoBuild ? "ON" : "OFF";
    ...
    if (ui.btnLoopSong) ui.btnLoopSong.onclick = () => {
        ...
        ui.loopState.textContent = globalSave.loopSong ? "ON" : "OFF";
        logSys(globalSave.loopSong ? "🔁 循環播放 ON" : "🔁 循環播放 OFF");
        saveGlobal();
    };
    // 廣告獎勵按鈕
    ...
    // 其他按鈕（暫時提示）
    if (ui.btnRobotSend) ui.btnRobotSend.onclick = () => logSys("🤖 機械人派遣功能開發中");
    if (ui.btnExchange) ui.btnExchange.onclick = () => logSys("💱 AENO兌換功能開發中");
    if (ui.btnTech) ui.btnTech.onclick = () => logSys("🔬 科技樹功能開發中");
...
function showPlanetSelect() {
    let planetOptions = "請選擇島嶼（1-20）：\n";
    for (let i = 1; i <= PLANET_COUNT; i++) {
        planetOptions += `${i}. 島嶼 ${i}\n`;
    }
...
function login(username, password) {
    ...
    if (username === "阿勒頓") {
        ...
        logSys("👑 開發者登入：阿勒頓 → 黑洞孤島");
    } else {
        globalSave.isDeveloper = false;
        logSys("👤 玩家登入：" + currentPlayer);
        // 選星球
