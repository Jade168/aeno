...
function parseJSONSafe(raw){
  ...
  function autoBuildTick(){
    ...
    else if(Math.random() < 0.3) choice = "tower";

    const def = BUILDINGS.find(b=>b.id===choice);
    if(!def) return;
    ...
    payCost(def.cost);
    placeBuildingAuto(choice, x, z);
    rebuildBuildings3D();

    logLine(`🤖 自動建造：${def.name}`);
  }
  ...
  function tryAutoLogin(){
    ...
    const key = SAVE_KEY_PREFIX + data.name;
  ...
  // ==========================
  async function boot(){
    bootProgress(20, "載入廣告資料...");
    await loadAds();

    bootProgress(40, "初始化登入系統...");
    initLogin();

    bootProgress(70, "準備遊戲引擎...");
